...
#define NETWORK_PACKET_TOTAL_SIZE (NETWORK_PACKET_NUMBER_SIZE + NETWORK_PACKET_MICROTIME_SIZE + NETWORK_PACKET_DATA_SIZE + MD5_SIZE_BYTES)

struct packet {
    uint16_t number;
    uint64_t microtime;
    int16_t data[NETWORK_PACKET_DATA_INT16_WORDS_COUNT];
    char md5[MD5_SIZE_BYTES];
};
